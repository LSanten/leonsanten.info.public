...
older_folder_2 = '/Users/lsanten/Documents/GitHub/MMS-Bubbles/Zettelkasten'  # Update this to the actual older folder path

print("Script started...")

print(f"CSV Path: {csv_path}")
print(f"Older Folder 2: {older_folder_2}")

print(f"Testing access to CSV: {os.path.exists(csv_path)}")
print(f"Testing access to older folder 2: {os.path.exists(older_folder_2)}")
...
def update_csv_with_older_files_2():
    """
    Updates an existing CSV by comparing it with markdown files from an even older folder.
    ...
    # Ensure the `found_in_older_2` column exists
    if 'found_in_older_2' not in fieldnames:
        fieldnames.append('found_in_older_2')
    ...
    older_files = [f for f in os.listdir(older_folder_2) if f.endswith('.md')]
    print(f"Markdown files in the even older folder: {older_files}")

    for file_name in older_files:
        marble_id = os.path.splitext(file_name)[0]
        older_file_path = os.path.join(older_folder_2, file_name)
        ...
        if marble_id in existing_ids:
            ...
            rows.append({
                ...
                'found_in_older_1': '',  # Preserve structure of older columns
                'found_in_older_2': 'NEW'
            })
